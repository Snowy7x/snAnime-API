...
function Mediafire(url, content) {
  ...
  var myRegEx =
    /href\s?=\s?.(https?:\/\/download\d{1,6}\.mediafire\.com.*?\.mp4)./g;
  var matches = getMatches(content, myRegEx, 1);
  for (let i = 0; i < matches.length; i++) {
    if (matches[i] !== undefined) {
      var link = matches[i];
...
function mixdrop(url, content) {
  ...
  if (detect(matches[0])) {
    ...
    var link =
      "https:" +
      getMatches(unpacked, /wurl=\"([^\"]+)/g, 1)[0].replace("\\", "");
    urls.push({
      file: link,
      label: "av",
    });
...
function cloudvideo(url, content) {
  var urls = [];
  var myRegEx =
    /source\ssrc="(https:.*?),(.*?),(.*?),[^"]*|source src="([^"]*)/g;
...
function vidfast(url, content) {
  ...
  var myRegEx =
    /sources:\s\[{file:"(https:.*?),(.*?),(.*?),\.urlset\/master\.m3u8"/g;
  var matches = myRegEx.exec(content);
  for (let i = 2; i < matches.length; i++) {
...
function get_chunks(str) {
  var chunks = str
    ? str.match(/eval\(\(?function\(.*?(,0,\{\}\)\)|split\('\|'\)\)\))($|\n)/g)
    : [];
